...
def build_vcd_model(component: str) -> None:
    ...
    # get component id
    resp = rs.get(
        "https://jira.lsstcorp.org/rest/api/2/project/LVV/components",
        auth=Config.AUTH,
    )
    cmps = resp.json()
    component_id = ""
    ...
    resp = rs.get(
        Config.VE_SEARCH_URL.format(cmpnt=component, maxR=max_res),
        auth=Config.AUTH,
    )
    ...
    for ve in veresp["issues"]:
        ...
        # print(i, tmp)
        tcraw = rs.get(
            Config.ISSUETCASES_URL.format(issuekey=tmp["jkey"]),
            auth=Config.AUTH,
        )
        tcrawj = tcraw.json()
        tmp["tcs"] = {}
    ...
    print(
        "\nGot",
        len(velem),
        "Verification Elements on",
        len(reqs),
        "Requirements. Found",
        len(Config.CACHED_TESTCASES),
        " related test cases.",
    )
    ...
    for tck in Config.CACHED_TESTCASES.keys():
        # print(tck, end="")
        tcd = rs.get(
            Config.TESTCASERESULT_URL.format(tcid=tck), auth=Config.AUTH
        )
        if tcd.status_code == 404:
            ...
            tctp = rs.get(
                Config.TESTPLANCYCLE_URL.format(trk=tcdj["key"]),
                auth=Config.AUTH,
            )
            tctpj = tctp.json()
    ...
    print(
        f"Number of Test Cases: & {len(Config.CACHED_TESTCASES)} \\\\",
        file=fsum,
    )
...
def get_tcs(veid: str) -> dict:
    ...
    # global tcases
    query = (
        "select tc.key, tc.FOLDER_ID, tc.LAST_TEST_RESULT_STATUS_ID, aos.name "
...
def get_ves(
    ...
    for ve in raw_ves:
        ...
        if ve[3] != "11713":  # ignore DESCOPED VEs
            ...
            query = (
                "select ji.issuenum, ji.summary from jiraissue ji "
                "inner join issuelink il on il.source = ji.id "
                "where destination = "
                + str(ve[1])
                + " and linktype = 10700 and ji.issuetype = 10602"
            )
            ...
            query = (
                ...
                "where ji.id = "
                + str(ve[1])
                + " and cf.id in (13511, 13703, 15204, 13513)"
            )
...
def get_tspec_r(fid: str) -> str:
    ...
    if not fid:
        return ""
    query = "select name, parent_id from AO_4D28DD_FOLDER where id = " + str(
        fid
    )
    # print(query)
    dbres = db_get(query)
...
def vcdsql(comp: str, RSP: str = "") -> list:
    ...
    if RSP != "":
        ...
        if len(spec_split) == 2:
            ...
            test_f = ""
        print(
            f"Test cases related to {RSP} grouped per priority.\n",
                ...
                for tc in ve["tcs"]:
                    ...
                    if tb == test_f or test_f == "":
                        ...
                        executed[tc_number] = lastR is not None and (
                            lastR["status"] == "passed"
                            or lastR["status"] == "cndpass"
                        )
                        if "Requirement Priority" in ve.keys():
                            by_priority[ve["Requirement Priority"]][
                                tc_number
                            ] = 1
                            # print(ve)
                        else:
                            print(
                                "No Req Priority for VE",
                                ve["jkey"],
                                ". Looking for VE priority.",
                            )
                            if (
                                "priority" in ve.keys()
                                and ve["priority"] != "Undefined"
                            ):
